...
def build_sparse_tensor(cond):
    voxel_coords = torch.argwhere(cond).long()
    spatial_shape = torch.Size((cond.shape[-1], cond.shape[-1], cond.shape[-1]))
    assert cond.dim() == 5
    voxel_grid_selected = cond[voxel_coords[:, 0], voxel_coords[:, 1], voxel_coords[:, 2], voxel_coords[:, 3], voxel_coords[:, 4]]
    voxel_coords = voxel_coords[:, [0, 2, 3, 4]]
    cond = spconv.SparseConvTensor(
        features=voxel_grid_selected.float().unsqueeze(-1),
...
class ModelBase(object):
    ...
    def _preprocess(self, batch, viz_flag=False, is_voxel=True):
        """
        ...
        data_inputs_centers = data["inputs.centers"]
        # B, T, 1, 32, 32, 32
        if is_voxel:
            B, T, _, _, _, _ = data_inputs.shape
            data_inputs = data_inputs.view(B*T, 1, 32, 32, 32)
            data_inputs_centers = data_inputs_centers.view(B*T, -1, 3)
            
            data_inputs = build_sparse_tensor(data_inputs)
            data["inputs_ult"] = {
                "cond": data_inputs,
                "centers": data_inputs_centers,
                "batch_size": B,
                "time_steps": T
            }
        
        data["inputs"] = data_inputs
        data["inputs.centers"] = data_inputs_centers
        batch = {"model_input": data, "meta_info": meta_info}
        # print(batch["model_input"].keys())
        # raise
        return batch
    ...
    def train_batch(self, batch, viz_flag=False, is_voxel=True):
        batch = self._preprocess(batch, viz_flag, is_voxel)
        self.set_train()
        self.zero_grad()
