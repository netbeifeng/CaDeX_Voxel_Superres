from .model_base_voxel import ModelBase
import torch
...
from core.net_bank.voxel_encoder import VoxelCondEncoder
...
class CaDeX_DFAU(torch.nn.Module):
    ...
    def forward(self, input_pack, viz_flag):
        ...
        seq_t, seq_pc, set_centers, set_ult = (
        ...
        if self.t_perm_inv:
            ...
            # else:
            c_t = self.network_dict["homeomorphism_encoder"](set_ult) # [B*T, 128]
            c_t = c_t.reshape(B, T, -1)
        else:
            _, c_t = self.network_dict["homeomorphism_encoder"](set_ult)  # B,C; B,T,C
