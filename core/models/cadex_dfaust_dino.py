from .model_base_dino import ModelBase
import torch
...
from core.net_bank.dino_encoder import DinoCondEncoder
...
class CaDeX_DFAU(torch.nn.Module):
    def __init__(self, cfg):
        ...
        if self.t_perm_inv:
            ...
            # homeomorphism_encoder = VoxelCondEncoder(
            #     input_dim=1,
            #     output_dim=128,
            # )
            homeomorphism_encoder = DinoCondEncoder(
                input_dim=1,
                output_dim=128,
            )
        else:
        ...
        self.network_dict = torch.nn.ModuleDict(
            {
                ...
                "canonical_geometry_encoder": ResnetPointnet(
                    dim=128, **cfg["model"]["canonical_geometry_encoder"]
                ),
    ...
    def forward(self, input_pack, viz_flag):
        ...
        seq_t, seq_pc = (
            ...
            input_pack["inputs"],
            # input_pack["inputs.centers"],
            # input_pack["inputs_ult"],
        )
        B, T= seq_t.shape
        # set_centers = set_centers.view(B, T, -1, 3)
        # encode Hoemo condition
        if self.t_perm_inv:
            # if is_voxel:
                
            # else:
            c_t = self.network_dict["homeomorphism_encoder"](seq_pc) # [B*T, 128]
        ...
        # inputs_cdc = self.map2canonical(c_t.transpose(2, 1), set_centers)  # B,T,N,3
        c_g = self.network_dict["canonical_geometry_encoder"](c_t)
        ...
        if viz_flag:
            ...
            # output["seq_pc"] = set_centers.detach()
